...
use Onlishop\Deployment\Tests\TestUtil\StaticSystemConfigHelper;
...
class AccountServiceTest extends TestCase
{
    ...
    #[Env(name: 'ONLISHOP_STORE_ACCOUNT_EMAIL', value: 'test@exaple.com')]
    #[Env(name: 'ONLISHOP_STORE_ACCOUNT_PASSWORD', value: 'password')]
    public function testShopSecretAdd(): void
    {
        $output = $this->createMock(SymfonyStyle::class);
        $output->expects($this->once())->method('info')->with(static::stringContains('Refreshed global shop token to communicate to store.onlishop.com'));
    ...
    public function testLoginPayloadChanged(): void
    {
        ...
        $this->expectExceptionMessage('Got invalid response from Onlishop API: {"message":"Invalid credentials"}');
        $service->refresh($output, '6.4.0-dev', 'old.example.com');
    }

    #[Env('ONLISHOP_STORE_SHOP_SECRET', 'some-key')]
