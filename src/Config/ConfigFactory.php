...
namespace Onlishop\Deployment\Config;
...
class ConfigFactory
{
    ...
    {
        $file = EnvironmentHelper::getVariable('ONLISHOP_PROJECT_CONFIG_FILE', $application->projectConfigFile);

        if ($file === null) {
            $file = Path::join($projectDir, '.onlishop-project.yml');

            if (!file_exists($file)) {
                $file = Path::join($projectDir, '.onlishop-project.yaml');
            }
        } else {
    ...
    {
        if (EnvironmentHelper::hasVariable('ONLISHOP_STORE_LICENSE_DOMAIN')) {
            $projectConfiguration->store->licenseDomain = EnvironmentHelper::getVariable('ONLISHOP_STORE_LICENSE_DOMAIN', '');
        }
